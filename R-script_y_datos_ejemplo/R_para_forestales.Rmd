...
#############################################################
# R para forestales - VirtualForests
# Aitor Vázquez Veloso e Irene Ruano Benito
# Junio, 2022
#############################################################
...
ifelse('plyr' %in% installed.packages(), print('paquete plyr instalado'), install.packages('plyr'))
library('plyr')
...
Para conocer mejor los campos existentes en ambos archivos se recomienda consultar el [documentador](https://www.miteco.gob.es/es/biodiversidad/servicios/banco-datos-naturaleza/documentador_bdsig_ifn3_tcm30-293905.pdf)

```{r carga de datos, echo=FALSE}

parcelas <- read.csv('parcelas_IFN3_ejemplo.csv')
arboles <- read.csv('arboles_IFN3_ejemplo.csv')
...
```{r variables de árbol, echo=FALSE}
...
# Cálculo del diámetro a la altura del pecho (dbh) promedio (cm). OJO, diámetros del IFN en mm.
...
# Cálculo del factor de expansión
#-# Dado que a la hora de tomar datos de un bosque se suelen elegir parcelas de muestreo, la superficie
#-# de la que se cuenta con datos de los árboles suele ser muy inferior a la hectárea. No obstante, los
#-# datos de la masa suelen expresarse a nivel de hectárea, lo que facilita la comparación enre inventarios
#-# y zonas de estudio. Para ello se crea el factor de expansión, que no es más que una variable que hace 
#-# referencia a la cantidad de árboles que representa, a nivel de hectárea, cada uno de los árboles de los
#-# que tenemos datos (parcela de muestreo). Su cálculo se hace por comparación de superficies entre la 
#-# hectárea y la superficie de la parcela de muestreo., de la siguiente manera:
...
#-# El caso del IFN es peculiar, dado que se muestrean parcelas concéntricas y, por lo tanto, el factor de 
#-# expansión de cada árbol depende de su posición dentro de la parcela de muestreo.
#-# Echa un vistazo a las parcelas del IFN: https://www.miteco.gob.es/es/biodiversidad/temas/inventarios-nacionales/c3_parcelasdecampo_tcm30-153778.pdf
...
#-# Esta variable representa el área basimétrica de los árboles cuyo diámetro es mayor al del árbol sujeto.
#-# Su cálculo consiste en la misma fórmula que el área basimétrica de la parcela, eliminando del cálculo
#-# (para cada árbol) al árbol estudiado y aquellos árboles que tienen un diámetro menor. Además. se tiene
#-# en cuenta el factor de expansión de cada árbol
...
for (parcela in unique(arboles$IFN3_ID)){ # para cada parcela...
        ...
        for (arbol in arboles_seleccionados$TREE_ID){ # para cada árbol...
            ...
            arbol_sujeto <- arboles_seleccionados[arboles_seleccionados$TREE_ID == arbol, ] # lo selecciono

            arbol_sujeto$bal <- bal # le asigno el bal
            lista_arboles_sujeto <- rbind(lista_arboles_sujeto, arbol_sujeto) # lo añado a la lista
            
            bal <- bal + arbol_sujeto$g_ha # actualizo el bal para el siguiente árbol de la parcela
                
        }
        
        resultados_bal <- rbind(resultados_bal, lista_arboles_sujeto) # añado los árboles de la parcela a resultados
}
...
coordenadas_parcelas <- parcelas[c('IFN3_ID', 'CoorX', 'Coory')] # extraemos las coordenadas de la parcela
coordenadas_parcelas <- coordenadas_parcelas[!duplicated(coordenadas_parcelas), ]
arboles <- merge(arboles, coordenadas_parcelas, by = 'IFN3_ID') # las unimos a los datos de árboles 
...
arboles$x_arbol_rel <- arboles$distancia*cos(arboles$rumbo_rad) # calculamos X relativa al centro de la parcela
arboles$y_arbol_rel <- arboles$distancia*sin(arboles$rumbo_rad) # calculamos Y relativa al centro de la parcela

arboles$x_arbol_abs <- arboles$distancia*cos(arboles$rumbo_rad) + arboles$X_parcela # calculamos X absoluta
arboles$y_arbol_abs <- arboles$distancia*sin(arboles$rumbo_rad) + arboles$Y_parcela # calculamos Y absoluta

rm(coordenadas_parcelas) # eliminamos la información temporal innecesaria
```
...
```{r variables de masa, echo=FALSE}

#-# Dado que la mayoría de cálculos que se hacen sobre la masa utilizan los datos de árboles, vamos a necesitar agrupar
#-# los árboles de cada una de las parcelas y realizar cálculos con ellos, tal y cómo se muestra a continuación
...
#-# Ahora, dado que hemos calculado la altura dominante en otra base de datos, la unimos con la base de datos anterior
datos_parcela$Ho <- merge(datos_parcela, # variable x a unir
...
#-# El sistema de cálculo es igual al de la altura dominante, pero en este caso utilizando el diámetro normal
...
.DiametroDominante_2 <- function(h, d, n){
        ...
        for(i in 1:length(d)){
                ncum = ncum + n[i]
                if(ncum>100) return(sum(d[1:i]*n[1:i], na.rm=TRUE)/sum(d[1:i]*n[1:i]/d[1:i], na.rm=TRUE))
        }
...
#-# Ahora, dado que hemos calculado la altura dominante en otra base de datos, la unimos con la base de datos anterior
datos_parcela$Do <- merge(datos_parcela, # variable x a unir
...
datos_parcela$S <- 10000/(datos_parcela$Ho*sqrt(datos_parcela$N))  # Índice de Hart-Becking
datos_parcela$S_tresbolillo <- (10000/datos_parcela$Ho)*sqrt(2/datos_parcela$N*sqrt(3))  # Índice de Hart-Becking para masas al tresbolillo
